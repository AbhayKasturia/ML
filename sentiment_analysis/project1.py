...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """    
    loss = label*(np.matmul(feature_vector, theta)+theta_0)
    if loss>=1:
        return 0
    else:
        return 1-loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    losses = labels*(np.matmul(feature_matrix, theta)+theta_0)
    avg_loss = 0
    for loss in losses:
        if loss>=1:
            avg_loss +=0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    return (current_theta_sum/total_run, current_theta_0_sum/total_run)
#pragma: coderesponse end
...
def classify(feature_matrix, theta, theta_0):
    ...
    """    
    res = np.matmul(feature_matrix,theta) + theta_0
...
def classifier_accuracy(
    ...
    pred_trained_values = classify(train_feature_matrix, theta, theta_0)    
    train_accuracy = accuracy(pred_trained_values, train_labels)
    
    pred_validation_values = classify(val_feature_matrix, theta, theta_0)    
    val_accuracy = accuracy(pred_validation_values, val_labels)
